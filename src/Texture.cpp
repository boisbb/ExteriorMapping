...
Texture::Texture(std::shared_ptr<Device> device, unsigned char* pixels, glm::vec2 dims)
    : m_device(device), m_dims(dims)
{
    VkDeviceSize imageSize = dims.x * dims.y * 4;
    ...
    m_image = std::make_shared<Image>(m_device, dims, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL,
